...
#                                                                             
# PROGRAMMER: 
# DATE CREATED: Artem Voronoff                                
# REVISED DATE: 06/04/2021
# PURPOSE: Creating a function classify_images that uses the classifier function 
...
def classify_images(images_dir, results_dic, model):
    ...
    # Popuylates dictionary with whether or not labels indicate a doc image (idx 3&4)
    for idx in range(0, len(filenames), 1):
    ...
    # Iteratesw through the list to print the results for each filename
    for key in results_dic:
        ...
        # Provides classification of the reults
        if sum(results_dic[key][2:]) == 3:
            print("*Breed Match*")
        if sum(results_dic[key][3:]) == 2:
            print("*Is-a-Dog Match*")
        if sum(results_dic[key][3:]) == 3 and results_dic[key][2] == 1:
